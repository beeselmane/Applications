...
{
    struct gbstate *gb = (struct gbstate *)malloc(sizeof(struct gbstate));
...
extern "C" void gb(int argc, char **argv)
{
    int fd = open(argv[1], O_RDONLY);
    if (!fd) return;
    close(fd);
    
    /*struct gbstate *gstate = (struct gbstate *)malloc(sizeof(struct gbstate));
